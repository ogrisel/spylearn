...
from spylearn.spylearn.histogram import histogram
import numpy as np
...
class TestHistogram(TestUtils):
    ...
    def test_bins_as_number():
        ...
        assert_array_almost_equals(np.array([0, 3, 6]), bin_edges)

    def test_bins_as_array():
        data = self.sc.parallelize([1, 2, 3, 4, 5])
    ...
    def test_ignore_out_of_range():
        data = self.sc.parallelize([1, 2, 3, 4, 5])
        hist, bin_edges = histogram(data, range=(2, 5), bins=2)
        assert_equals(2, hist[0])
        assert_equals(1, hist[1])
        assert_array_almost_equals(np.array([2, 3.5, 5]), bin_edges)
